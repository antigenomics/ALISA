...
/* Taken from https://github.com/EsotericSoftware/kryo */
...
public class IntMap {
    ...
    public IntMap(int initialCapacity, float loadFactor) {
        ...
        valueTable = (double[]) new Object[keyTable.length];
    }

    /**
     * Creates a new map identical to the specified map.
     */
    public IntMap(IntMap<? extends double> map) {
    ...
    public double put(int key, double value) {
        ...
        if (key1 == key) {
            double oldValue = valueTable[index1];
        ...
        if (key2 == key) {
            double oldValue = valueTable[index2];
        ...
        if (key3 == key) {
            double oldValue = valueTable[index3];
        ...
        if (isBigTable) {
            ...
            if (key4 == key) {
                double oldValue = valueTable[index4];
    ...
    public void putAll(IntMap<double> map) {
        for (Entry<double> entry : map.entries())
    ...
    private void push(int insertKey, double insertValue, int index1, int key1, int index2, int key2, int index3, int key3, int index4,
                      int key4) {
        // avoid getfield opcode
        int[] keyTable = this.keyTable;
    ...
    public double get(int key) {
        if (key == 0) {
            if (!hasZeroValue) return Double.NaN;
            return zeroValue;
        }
        int index = key & mask;
        if (keyTable[index] != key) {
            ...
            if (keyTable[index] != key) {
                ...
                if (keyTable[index] != key) {
                    if (isBigTable) {
                        index = hash4(key);
                        if (keyTable[index] != key) return getStash(key, Double.NaN);
                    } else {
                        return getStash(key, Double.NaN);
                    }
                }
            }
        }
    ...
    private double getStash(int key, double defaultValue) {
        int[] keyTable = this.keyTable;
        for (int i = capacity, n = i + stashSize; i < n; i++)
    ...
    void removeStashIndex(int index) {
        ...
        if (index < lastIndex) {
            ...
            valueTable[lastIndex] = Double.NaN;
        } else
            valueTable[index] = Double.NaN;
    }
     ...
     *                 {@link #equals(Object)}.
     */
    public boolean containsValue(double value, boolean identity) {
        double[] valueTable = this.valueTable;
        if (value == null) {
            if (hasZeroValue && zeroValue == Double.NaN) return true;
            int[] keyTable = this.keyTable;
            for (int i = capacity + stashSize; i-- > 0; )
    ...
    private void resize(int newSize) {
        ...
        valueTable = (double[]) new Object[newSize + stashCapacity];
