...
class DenseVectorTest {
    ...
    void instanceTest() {
        ...
        assertEquals(
                Vector.DENSE(1, 2, 0, 3, 4).getEffectiveSize(),
                4);
        assertEquals(
                Vector.DENSE(1, 2, 0, 3, 4).length,
                5);
    }
    ...
    void linearOperationsTest() {
        // commutative
        Vector v1 = Vector.DENSE(0, 7, 1, -2, 100),
                v2 = Vector.DENSE(0, 7, 100, 0.1, -10);

        assertTrue(v1.add(v2).equals(v2.add(v1)));

        assertEquals(
                Vector.DENSE(6, 3, 5, 7, 9),
                Vector.DENSE(5, 1, 2, 3, 4).add(Vector.DENSE(1, 2, 3, 4, 5))
        );

        assertEquals(
                Vector.DENSE(2.5, 0.5, 1, 1.5, 2),
                Vector.DENSE(5, 1, 2, 3, 4).multiply(0.5)
        );
    }

    @Test
    void linearOperationsInplaceTest() {
        Vector v1 = Vector.DENSE(1, 20, -3, 4, 1.1231232e-10);
    ...
    void algebraTest() {
        ...
        Vector v1 = Vector.DENSE(1, -2, 0, -3, 4, 10, 12, 100, -10);
